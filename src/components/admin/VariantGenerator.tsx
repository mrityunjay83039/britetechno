    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {options.map((option, optIdx) => (
          <div
            key={optIdx}
            className="p-4 bg-[#FAF8F5] border border-[#C5A880]/20 rounded-sm space-y-3"
          >
            <div className="flex items-center justify-between">
              <span className="font-sans text-xs font-bold text-[#0F0F11] uppercase tracking-wider">
                Option Name: <span className="text-[#FF6F61]">{option.name}</span>
              </span>
              <button
                type="button"
            ...
            <div className="flex flex-wrap items-center gap-2">
              {option.values.map((val, valIdx) => (
                <span
                  key={valIdx}
                  className="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-[#C5A880]/30 text-xs font-sans text-[#0F0F11] font-semibold rounded-xs shadow-2xs"
                >
                  {val}
                  <button
                    type="button"
                    onClick={() => handleRemoveValue(optIdx, valIdx)}
                    className="text-[#8C857B] hover:text-red-600 transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="px-2.5 py-1 text-xs font-sans bg-white border border-[#C5A880]/30 focus:border-[#C5A880] rounded-xs outline-none"
                />
                <button
                  type="button"
                  onClick={() => handleAddValue(optIdx)}
                  className="px-2.5 py-1 bg-[#0F0F11] text-white hover:bg-[#C5A880] hover:text-[#0F0F11] text-xs font-sans font-bold uppercase rounded-xs transition-colors cursor-pointer"
                >
                  Add
                </button>
              </div>
      ...
      {/* Add New Option Bar */}
      <div className="flex items-center gap-2 p-3 bg-white border border-[#C5A880]/20 rounded-sm">
        <input
          type="text"
          value={newOptionName}
          ...
          className="flex-1 px-3 py-2 text-xs font-sans bg-white border border-[#C5A880]/30 focus:border-[#C5A880] rounded-sm outline-none"
        />
        <button
          type="button"
          onClick={handleAddOption}
          className="px-4 py-2 bg-[#0F0F11] text-white hover:bg-[#C5A880] hover:text-[#0F0F11] text-xs font-sans font-bold uppercase tracking-wider rounded-sm transition-colors cursor-pointer flex items-center gap-1"
        >
      ...
      <div className="flex justify-end">
        <button
          ...
          className="text-xs font-sans font-bold text-[#FF6F61] hover:text-[#0F0F11] flex items-center gap-1.5 cursor-pointer uppercase tracking-wider transition-colors"
        >
